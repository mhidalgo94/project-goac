...
$(function () {
    ...
    $('#form-0').on('submit', function (e) {
        ...
        Swal.fire({
            title: 'Confirmacion',
            text: "Estas seguro de crear esta investigacion",
            icon: 'info',
            ...
            if (result.value) {
                $.ajax({
                    ...
                    if (!data.hasOwnProperty('error')) {
                        Swal.fire({
                            title: 'Notificación!',
                            text: 'Investigacion ha sido creado correctamente',
                            icon: 'success',
                            timer: 3000,
                            onClose: () => {
    ...
    $('#form-1').on('submit', function (e) {
        ...
        Swal.fire({
            title: 'Confirmación',
            text: "Estas seguro de agregar nueva Categoría Investigación",
            icon: 'info',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            ...
            if (result.value) {
                $.ajax({
                    ...
                    if (!data.hasOwnProperty('error')) {
                        Swal.fire({
                            title: 'Notificación!',
                            text: 'Nueva Categoría creada correctamente',
                            icon: 'success',
                            timer: 3000,
                            onClose: () => {
                                location.reload();
