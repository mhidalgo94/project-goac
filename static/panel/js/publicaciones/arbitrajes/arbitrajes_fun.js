            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    $.each(row.miembro, function (key,value) {
                       html += '<span class="tag has-background-white"><img src="'+value.imagen+'" class="image mt-2" style="width: 48px; height: 48px;"></span>'; 
                    });
                    return html;
...
$(function (){
    ...
    $('#dtable').on('click', 'button[rel="delete"]' , function(){
        ...
        confirm(window.location.pathname, 'Advertencia','Estas seguro que deseas eliminar este Artículo?','warning' ,parameters, function(){
            Swal.fire({
                title : 'Notificación!',
                text : 'Artículo ha sido eliminado correctamente',
                icon: 'success',
                timer: 3000,
                onClose: () => {
                    updatetable();
    ...
    $('#dtable').on('click', 'button[rel="edit"]', function(){
        ...
        location.href = "/dashboart/publicaciones/arbitraje/edit_arbitraje/"+id_+"/";
    });
