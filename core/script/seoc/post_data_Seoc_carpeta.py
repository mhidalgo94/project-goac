...
NAME_FILE_LOG = 'seoc_log_carpeta.txt'
...
def Datos_Seoc():
    try:
        list_file = os.listdir(ACTINO_ROOT)
        print(len(list_file))
        for f in list_file:
            datos_csv = Datos_csv(f)
            if datos_csv:
                info = datos_csv['info']
                fecha = info['fecha']
                ...
                for d in datos:
                    ...
                    seoc= SEOC(estacion=f[0] ,fecha = fecha, presion_estacion=1008.9, presion_admosferica=1013.25,nubosidad=nubosidad, iDirecta=iDirecta, czsa=czsa, dSolar=dSolar)
                    dicc_data = seoc.get_data()
                    dicc_data['tiempo'] = horario
                    dicc_data['fecha'] = formato_fecha_bd(fecha)
                    dicc_data['estacion'] = ESTACIONES[f[0]]
                    request_post(dicc_data)
            else:
                texto = 'El archivo no existe en la carpeta'
                save_log(texto)
            print('Envio datos Terminado') # Cuando termina de leer el archivo
