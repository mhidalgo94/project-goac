...
# Estructura url para envio de datos
URL_ROOT = 'http://10.6.107.7'
...
# Si activa logs debe configurar el path LOG_ROOT
ACTIVE_LOG = True
LOG_ROOT = '/home/goac/goac-app/scripts/logs'
NAME_FILE_LOG = 'met_log_carpeta.txt'
...
# Vincula los datos met con diccionario KEY
# Luego envia los datos a la web
def vinc_data(*lista):
    '''
    ...
    fecha = fecha_.strftime("%Y-%m-%d %H:%M:%S")
...
def iniciar_():
    try:
        list_file = os.listdir(MET_ROOT)
        for archivo in list_file:
            lectura_archivo(archivo)
            print('Envio datos Terminado')
